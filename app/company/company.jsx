...
import DiscountedCashflow from './dcf.jsx'
...
var Company = React.createClass({
  ...
  fetchCompany: function(params) {
    ...
    Api.get(Api.company(exc, con)).then(function(response) {
      const schedules = Api.schedules(response.id);
      Api.get(schedules.path, schedules.params)
      .then(investActivity => {
        setTitle(response.name);
        response.number_set.cashflow.push(...investActivity);
        this.setState({
          company: response,
          comparisons: []
        });
  ...
  render: function() {
    ...
    return <div>
      ...
      <section id="dcf">
        <DiscountedCashflow company={company} />
      </section>
    </div>
  }
});
